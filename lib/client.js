...
/**
 * "Ascii-armor" the given credentials.  This amounts to JSON-encoding them,
 * encoding with base64, breaking into lines, and adding a header and footer.
 * The unarmorCredentials function does the inverse, trying to be nice along
 * the way regarding extra garbage and whitespace.
 */
var header = "-----BEGIN TASKCLUSTER CREDENTIALS-----",
    footer = "-----END TASKCLUSTER CREDENTIALS-----";
exports.armorCredentials = function(credentials) {
  var clientId = credentials.clientId;

  credentials = JSON.stringify(credentials);
  credentials = new Buffer(credentials).toString('base64');
  credentials = credentials.match(/.{1,64}/g);
...
exports.unarmorCredentials = function(armored) {
  armored = armored.split(/\r?\n/).map(function(l) { return l.trim(); });

  // strip leading and trailing garbage and header/footer
  while (armored.length && armored[0] !== header) {
    armored.shift();
  }
  armored.shift();

  while (armored.length && armored[armored.length-1] != footer) {
    armored.pop();
  }
  armored.pop();

  if (!armored) {
    throw new Error("Invalid armored credentials: no header or footer found");
  }

  // parse header
  while (armored.length && armored[0] != '') {
    var h = armored[0].split(': ', 2);
    if (!h[1]) {
      throw new Error("Invalid armored credentials: invalid header");
    }
    if (h[0] === "Version") {
      if (h[1] !== "1") {
        throw new Error("Invalid armored credentials: unrecognized version");
      }
    } else if (h[0] == "ClientId" || h[0] === "Caution") {
      // (ignored)
    } else {
      throw new Error("Invalid armored credentials: Unrecognized header");
    }
    armored.shift();
  }
  if (armored) {
  ...
  // the remainder is the base64-encoded body
  var credentials = new Buffer(armored.join(''), 'base64').toString('ascii');
