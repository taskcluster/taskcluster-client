 ...
 * }
 */
function buildPulseConnectionString(options) {
...
/**
@param {Object|String} connectionString for pulse connection. Either an amqp uri
  or an object (see buildPulseConnectionString)
*/
var PulseConnection = function(connectionString) {
  assert(connectionString, "connectionString is required");

  if (typeof connectionString === 'object') {
    connectionString = buildPulseConnectionString(connectionString);
  }

  this._connectionString = connectionString;

  // Parse out the username which we need to figure out more complex things...
  this.username = '';
  var parsed = URL.parse(connectionString);
  if (parsed.auth) {
    this.username = parsed.auth.split(':')[0];
  }
