...
var PulseConnection = function(options) {
  ...
  options = _.defaults({}, options, {
    ...
    protocol:             options.port === 5672 ? 'amqp' : 'amqps',
  ...
  if (!options.namespace) {
    options.namespace = url.parse(options.connectionString).auth.split(':')[0];
  }
...
PulseConnection.prototype.connect = function() {
  ...
  var connect = function() {
    return new Promise(function(accept) {
      var attempts = self._reconnectAttempts;
      if (attempts > 0) {
        // Construct the usual delay and retry after sleeping
        var delay = Math.pow(2, attempts) * self._options.delayFactor;
        var rf = self._options.randomizationFactor;
        delay = delay * (Math.random() * 2 * rf + 1 - rf);
        delay = Math.min(delay, self._options.maxDelay);
        return setTimeout(accept, delay);
      }
      return accept();
    }).then(function() {
      self._lastReconnect = Date.now();
      return amqplib.connect(self._options.connectionString);
    }).then(function(conn) {
      // Save reference to the connection
      self._conn = conn;

      // Setup error handling
      conn.on('error', self._onError.bind(self));

      // Return connection
      return conn;
    }, function(err) {
      // Retry connection if we're not out of time
      self._rethrowOrRetry(err);
      return connect();
    });
  };
  ...
  if (options.reconnect) {
    this._connection.on('reconnect', function() {
      if (that._channel) {
        ...
        that._channel = null;
        var done = that._consumerTag ? that.resume() : that.connect();
...
PulseListener.prototype.pause = function() {
  if (!this._channel || !this._consumerTag) {
    debug("PulseListener.pause(): Not connected or not consuming yet...");
...
PulseListener.prototype.deleteQueue = function() {
  ...
  return this.pause().then(function() {
    return self.connect().then(function(channel) {
      return channel.deleteQueue(self._queueName).then(function() {
        return self.close();  //TODO: fix closing when we have retries!!!
      });
    });
  });
};
